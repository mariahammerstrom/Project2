...
\begin{center}
\begin{tabular}{ l l l }\hline
	$\omega_r$ 			& $\lambda_0$			& $\epsilon_0^{'}$	 	\\ \hline
...
For the case of $\omega_r = 0.01$, the chosen value of $\rho_{max}$ is important. $\rho_{max}$ needs to be quite large to make the plot come out right. We landed on $\rho_{max} = 40$.
